        ...
        while not f == -1:
            ...
            while index < len(s):
                ...
                if next_cha.isdigit():
                    ...
                    if next_cha.find("."):
                        print(
                            "more than one decimal points in angle measurement behind "
                            + t
                        )
                        return s
                    angle_measure += next_cha
...
def multiply_brackets(number_string):
    ...
    if indeces_of_open_brackets:
        ...
        number_string = number_string[0] + replace_by_index(
            number_string[1:], "*(", *indeces_of_open_brackets
        )
    ...
    if indeces_of_close_brackets:
        # adding "*" before "(" only if it should be added
        number_string = number_string[0] + replace_by_index(
            number_string[1:], ")*", *indeces_of_close_brackets
        )
...
def calculate(input):
    if input:
        ...
        if contains_Alpha_Or_SpecCha(
            input
        ):  # if the user's input contains alphabets or other characters that are not mathematical notations
            if containsTrigoSigns(input):  # if the input has trigo signs
                try:
                    number_only = calculateTrigo(
                        input
                    )  # calculate the trigo functions and store only numbers and operators
                except ValueError:
            ...
            if isinstance(e, TypeError):
                ...
                try:
                    ...
                    try:
                        ...
                        result = eval(
                            new_number_only
                        )  # evaluating the user's math problem

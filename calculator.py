...
async def command_calculate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    input = (
        str("".join(context.args)).replace(" ", "").lower()
    )  # removing all spaces in the input
    input = input.replace("[]", "")
...
async def inline_calculate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    input = (
        str("".join(query)).replace(" ", "").lower()
    )  # removing all spaces in the input
    after_calculating = myFunctions.calculate(input)
    # if the query could be evaluated, after_calculating should be a tuple
    if isinstance(after_calculating, tuple):
        result = after_calculating[
            1
        ]  # this will be the result of the user's math problem
        message = result
    else:
        if query:
...
async def unknown(update=Update, context=ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text="Sorry I do not understand this command"
    )
...
if __name__ == "__main__":
    ...
    application = (
        ApplicationBuilder()
        .token("7490282882:AAGRs2KGK-NdUGZW5pmq-i-71BZzBDaMHes")
        .build()
    )
    ...
    calculate_msg_handler = MessageHandler(
        filters.TEXT & (~filters.COMMAND), msg_calculate
    )
